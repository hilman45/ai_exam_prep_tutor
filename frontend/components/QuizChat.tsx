...
export default function QuizChat({ 
  ...
  // Reset messages when question changes (only if question is provided)
  useEffect(() => {
  ...
  const getCorrectAnswerText = () => {
    ...
    return question.options[question.answer_index] || null
  }

  const getUserAnswerText = () => {
    if (!question || userAnswer === null || userAnswer === undefined) return null
    const answerIndex = userAnswer as number
    if (answerIndex < 0 || answerIndex >= question.options.length) return null
  ...
  const handleSend = async () => {
    ...
    try {
      // Call backend API - handles both specific question and general quiz context
      const reply = await chatService.sendQuizMessage(
        userMessage.content,
        question ? question.question : null,
        question ? question.options : null,
        question ? getCorrectAnswerText() || null : null,
        question ? getUserAnswerText() : null,
        topicName || null,
        explanation || null,
        allQuestions || null,
        quizName || null
              ...
              <div className="text-center text-gray-500 mt-8">
                ...
                <p className="text-sm">
                  {question 
                    ...
                    : "Ask me anything about this quiz or topic!"}
                </p>
                <p className="text-xs text-gray-400 mt-2">
                  {question 
                    ? 'Try: "Why is this answer wrong?" or "Explain the correct answer"'
                    : 'Try: "Explain the main concepts" or "What topics does this quiz cover?"'}
                </p>
              </div>
            ) : (
